...
describe('TelegramChannel', () => {
  ...
  describe('document attachment download', () => {
    it('downloads document and includes attachment marker in content', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledTimes(1);
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.content).toContain('[Document: report.pdf]');
      expect(storedMsg.content).toContain('(__attachment__:');
    });
    ...
    it('falls back to placeholder on download failure', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledTimes(1);
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.content).toBe('[Document: report.pdf]');
      expect(storedMsg.content).not.toContain('(__attachment__:');
    });
    ...
    it('preserves caption alongside attachment marker', async () => {
      ...
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.content).toContain('[Document: draft.pdf]');
      expect(storedMsg.content).toContain('(__attachment__:');
      expect(storedMsg.content).toContain('Please review this');
    });

    it('passes thread_id through for documents', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.thread_id).toBe('42');
    });
  });

  describe('photo attachment download', () => {
    it('downloads largest photo resolution and includes marker', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledTimes(1);
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.content).toContain('[Photo]');
      expect(storedMsg.content).toContain('(__attachment__:');
      expect(storedMsg.content).toContain('501-photo.jpg');
    });

    it('falls back to placeholder on photo download failure', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledTimes(1);
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.content).toBe('[Photo]');
      expect(storedMsg.content).not.toContain('(__attachment__:');
    });
    ...
    it('includes caption with photo marker', async () => {
      ...
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.content).toContain('[Photo]');
      expect(storedMsg.content).toContain('Check this out');
      expect(storedMsg.content).toContain('(__attachment__:');
    });

    it('passes thread_id through for photos', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      const storedMsg = (opts.onMessage as any).mock.calls[0][1];
      expect(storedMsg.thread_id).toBe('55');
    });
  });
});

...
describe('validateDrafts', () => {
  ...
  it('reports missing source note', () => {
    ...
    const result = validateDrafts(DRAFTS, JOB_ID, 'paper.pdf');
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.check === 'source-note-exists')).toBe(
      true,
    );
  });
  ...
  it('reports missing concept note files', () => {
    ...
    const result = validateDrafts(DRAFTS, JOB_ID, 'paper.pdf');
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.check === 'concept-note-exists')).toBe(
      true,
    );
  });
